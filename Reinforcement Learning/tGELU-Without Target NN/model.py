...
class Actor(nn.Module):
    def __init__(self, env, config):
        ...
        self.optimizer = config['optimizer'](self.parameters(),lr = config['lr_q'])
...
class Agent:
    ...
    def update(self, buffer,batch_size = 256, gradient_steps = 1, q_freq = 1):
        ...
        for n_updates in range(1,gradient_steps+1):                       
            ...
            if(n_updates % q_freq == 0):
                ...
                with torch.no_grad():
                    _ , next_q_val = self.actor.get_action(next_state, deterministic = True)                 
    ...
    def reduce_grad(self): 
        with torch.no_grad():
            norm = 0
            for param in self.actor.q_net[4].parameters():
    ...
    def evaluate(self, epi_len= 1000, n_iter = 1, deterministic = True):
        ...
        with torch.no_grad():
            ...
            for epi in range(n_iter):
                ...
                epi_step = 0
                gamma_eval = 1
                while(not done and epi_step < epi_len):
                    total_steps += 1
                    epi_step+=1
                    action = self.get_action(torch.tensor(state).float(), deterministic = deterministic)
                    #print("eval action",action)
                    next_state, reward, done, _, _ = self.env.step(int(action.detach().item()))
                    total_reward += reward
                    discounted_reward += gamma_eval*reward
                    gamma_eval = gamma_eval*self.gamma
                    state = next_state                            
        
        return (total_reward/n_iter), (total_reward/total_steps), (discounted_reward/n_iter) 
